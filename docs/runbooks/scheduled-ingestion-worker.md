...
- 生产仅在 `PROVIDER_MODE=live` 时注册 `cn.daily-bars`、必需的 `hk.core-index-bars`、可选的
  `hk.equity-bars`、`us.daily-bars`、`cn.macro-release-calendar`、`hk.macro-release-calendar`、
  `us.macro-release-calendar`、`cn.official-headlines`、`hk.official-headlines`；它们分别调用 BaoStock、
  XtQuant、Twelve Data、NBS 发布日程、C&SD 年度日程、OMB/BLS + BEA 官方日程、NBS 数据发布列表和
  HKMA 的 checkpointed handler。fixture role 不会注册。
