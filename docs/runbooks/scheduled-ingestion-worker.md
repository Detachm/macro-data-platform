...
- 生产仅在 `PROVIDER_MODE=live` 时注册 `cn.daily-bars`、`hk.daily-bars`、`us.daily-bars`；它们分别
  调用 BaoStock、XtQuant、Twelve Data 的 checkpointed handler。fixture role 不会注册。
...
CN/HK news 和 CN/US macro release 尚未拥有 live checkpointed ingestion handler。materializer 若
不能从事实库得到它们的合格数据，会将相关必需输入写为 `missing`，报告质量为 `blocked`；这是预期的
