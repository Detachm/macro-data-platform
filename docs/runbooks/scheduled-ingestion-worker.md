...
- `ReportInputQualityGate` 只检查完整性、时效、隔离、修订与临时错误。来源权利、引用权限和
  external-LLM 标记不是运行时 gate；见 ADR 0005。
...
日志事件 `scheduled_task_finished` 含 `report_date`、`task_id`、`provider_role`、`attempt_no`、
`run_id`、`terminal` 和 `error_code`。Prometheus 指标为
