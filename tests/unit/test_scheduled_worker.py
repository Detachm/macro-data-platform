...
from macro_platform.jobs.quality_gate import ReportInputQualityGate
...
def test_rpt_029_legacy_rights_marker_does_not_create_a_rights_gate() -> None:
    ...
    assert [(issue.input_id, issue.code) for issue in result.issues] == [
        ...
        ("news.cn.official_headlines_24h", "INVALID_REQUIRED_INPUT_STATUS"),
    ]
...
class _Task:
    task_id: str
    required: bool = True
    outcomes: list[Exception | None] = field(default_factory=lambda: [None])
