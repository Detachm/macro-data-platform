"""Durable ingestion bridge for BaoStock CN core-index daily bars."""
...
from macro_platform.providers.cn.baostock import BAOSTOCK_PRIMARY_ROLE, BaoStockDailyBarsProvider
...
class CnBaoStockIngestHandler:
    """Persist one checkpointed BaoStock page and its source-time audit data."""
    ...
    async def run(self, request: IngestJobRequest) -> IngestJobResult:
        raise RuntimeError("BaoStock ingestion must run through JobRunner with a database")
    ...
    async def run_checkpointed(
        ...
        for warning in page.warnings:
            ...
            PROVIDER_REJECTIONS.labels(
                provider_role=BAOSTOCK_PRIMARY_ROLE,
    ...
    def _validate_request(request: IngestJobRequest) -> None:
        if request.provider_role != BAOSTOCK_PRIMARY_ROLE:
            raise ValueError("BaoStock ingestion requires the cn.bars.primary role")
        if request.dataset is not Dataset.BARS:
            raise ValueError("BaoStock ingestion supports bars only")
        if request.regions != {Region.CN}:
            raise ValueError("BaoStock ingestion requires the CN region")
