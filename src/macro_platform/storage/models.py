...
class WorkflowAlertAttemptRow(Base):
    __tablename__ = "workflow_alert_attempts"
    __table_args__ = (
        UniqueConstraint("idempotency_key", name="uq_workflow_alert_idempotency"),
        CheckConstraint(
            "status IN ('pending', 'succeeded', 'failed', 'retry_wait', 'uncertain')",
            name="ck_workflow_alert_attempts_status",
        ),
        Index("ix_workflow_alerts_report_status", "report_date", "status"),
        Index("ix_workflow_alerts_run", "workflow_run_id"),
    )

    alert_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True)
    workflow_run_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True))
    report_date: Mapped[date] = mapped_column(Date)
